...
<div class="test-container span6 offset3">
 	<div class="test">
 		<div class="row-fluid">
 			@if($prev)
        	<div class="span6">
        	{{ HTML::link('session/test/'.$session->id.'/'.$prev->test_id, '&larr; Previous Test.',  array('class'=>'previous-test')) }}
        	</div>
        	@endif
        	@if($next)
        	<div class="span6 pull-right align-right">
        	{{ HTML::link('session/test/'.$session->id.'/'.$next->test_id, 'Next Test. &rarr; ', array('class'=>'next-test')) }}
        	</div>
        	@endif
        </div>
